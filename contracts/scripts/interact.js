...
async function main() {
    ...
    // Get signers
    const [owner, voter1, voter2, candidate1, candidate2] = await hardhat_1.ethers.getSigners();
    ...
    console.log("\nCreating election...");
    await votingSystem.createElection("Student Council Election 2024", "Annual student council election for the upcoming academic year", startTime, endTime);
    console.log("Election created successfully!");
    // Register voters
    const voterFee = await votingSystem.registrationFee();
    console.log(`\nRegistering voters (Fee: ${hardhat_1.ethers.formatEther(voterFee)} ETH)...`);
    ...
    console.log(`\nRegistering candidates (Fee: ${hardhat_1.ethers.formatEther(candidateFee)} ETH)...`);
    await votingSystem.connect(candidate1).registerCandidate(1, "Alice Johnson", "Progressive Students", "President", "QmHashAlice123", { value: candidateFee });
    await votingSystem.connect(candidate2).registerCandidate(1, "Bob Smith", "Conservative Students", "President", "QmHashBob456", { value: candidateFee });
